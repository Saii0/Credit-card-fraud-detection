...
R1 = tk.Radiobutton(ttop, text="Add Mode", variable=Var, value=1,
  command=sel,bg='#d3d3d3',fg='brown')
...
MMCodeLB = tk.Label(ttop, text='Merchant Code : ',bg='#d3d3d3')  # More labels
MMNameLB = tk.Label(ttop, text='Merchant Name : ',bg='#d3d3d3')  # ^
MMStateLB = tk.Label(ttop, text='State : ',bg='#d3d3d3')  # ^
MMSiteLB = tk.Label(ttop, text='Site : ',bg='#d3d3d3')  # ^

MMInfoLB = tk.Label(ttop, text='-------',fg='red',bg='#d3d3d3',font=('times', 15,' bold '))  # ^


#Set up text entry boxes

MMCodeEL = tk.Entry(ttop, textvariable = MMCode)  # The entry input
MMNameEL= tk.Entry(ttop, textvariable = MMName,width=40)  # The entry input
MMStateEL = tk.Entry(ttop, textvariable = MMState)    
...
#    customerData = [(None, MMCode.get(), MMNam.get(), MMState.get(), MMSite.get())]
    customerData = [(None, MMCodeEL.get(), MMNameEL.get(),MMStateEL.get(), MMSiteEL.get())]
    if Mode:
        ...
        c.execute("""UPDATE Merchant_master SET MMName='{0}',MMState={1},
                  MMSite={2} WHERE MMCode='{3}' """.format(MMNameEL.get(), MMStateEL.get(),
                  MMSiteEL.get(),MMCodeEL.get()))
        conn.commit()
...
#    sqlstr="select TMCode from Trans_master WHERE Id=( Select MAX(id) FROM Trans_master)"
    MMc=MMCodeEL.get()
#    print(MMc)
    sqlstr="select MMName,MMState, MMSite, from Merchant_master WHERE MMCode='{0}'".format(MMc)
#    print(sqlstr)
    c.execute(sqlstr)
